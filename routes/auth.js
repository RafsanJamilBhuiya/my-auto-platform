...
router.post('/register',express.json(),async(req,res)=>{
  try{
    const email=String(req.body.email||'').trim().toLowerCase(); const password=String(req.body.password||''); const name=String(req.body.name||'').trim()||email.split('@')[0];
    ...
    const created=await store.createUser(user); issue(res,created); res.status(201).json({ok:true,user:safeUser(created)});
  }catch(error){console.error('[Auth] register:',error);res.status(500).json({ok:false,error:'Registration failed'});}
});

router.post('/login',express.json(),async(req,res)=>{
  try{
    const email=String(req.body.email||'').trim().toLowerCase(); const password=String(req.body.password||''); const user=await store.findUserByEmail(email);
    if(!user||!user.password_hash||!user.salt)return res.status(401).json({ok:false,error:'Invalid credentials'});
    const candidate=Buffer.from(crypto.scryptSync(password,user.salt,64).toString('hex'),'hex'); const expected=Buffer.from(user.password_hash,'hex');
    ...
    issue(res,user); res.json({ok:true,user:safeUser(user)});
  }catch(error){console.error('[Auth] login:',error);res.status(500).json({ok:false,error:'Login failed'});}
});

router.post('/logout',(req,res)=>{res.clearCookie('auth_token',{httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',path:'/'});res.json({ok:true});});

// Google Identity Services credential endpoint. No Google client secret or server-side OAuth bootstrap is required.
router.post('/google/credential',express.json(),async(req,res)=>{
  try{
    ...
    // Google tokeninfo validates the signed ID token and returns its trusted claims.
    ...
    if(expectedClientId && profile.aud!==expectedClientId)return res.status(401).json({ok:false,error:'Google credential audience mismatch'});
    ...
    issue(res,user); res.json({ok:true,user:safeUser(user)});
  }catch(error){console.error('[Auth] Google credential:',error);res.status(502).json({ok:false,error:'Google sign-in verification failed'});}
});

// Legacy OAuth redirect is retained as a compatibility fallback when a Google client secret is configured.
router.get('/google',(req,res)=>{
  const clientId=process.env.GOOGLE_CLIENT_ID, secret=process.env.GOOGLE_CLIENT_SECRET;
  if(!clientId||!secret)return res.status(404).json({ok:false,error:'Use Google Identity Services sign-in on the login page.'});
...
router.get('/google/callback',async(req,res)=>{
  ...
  const stateCookie=req.cookies?.google_oauth_state, verifier=req.cookies?.google_oauth_verifier, received=String(req.query.state||'');
  ...
  try{
    ...
    if(!tokenResponse.ok)throw new Error('Google token exchange failed'); const tokens=await tokenResponse.json();
    const profileResponse=await fetch('https://openidconnect.googleapis.com/v1/userinfo',{headers:{authorization:`Bearer ${tokens.access_token}`}}); if(!profileResponse.ok)throw new Error('Google profile lookup failed');
    const profile=await profileResponse.json(); const email=String(profile.email||'').trim().toLowerCase(); if(!profile.email_verified)throw new Error('Google email is not verified');
    let user=await store.findUserByEmail(email); if(!user)user=await store.createUser({id:crypto.randomUUID(),email,name:profile.name||email.split('@')[0],role:(await store.countUsers())===0?'admin':'user',provider:'google',salt:null,password_hash:null});
    issue(res,user); res.clearCookie('google_oauth_state',{path:'/api/auth/google'});res.clearCookie('google_oauth_verifier',{path:'/api/auth/google'});res.redirect('/dashboard?auth=success');
  }catch(error){console.error('[Auth] Google OAuth:',error);res.status(502).send('Google OAuth failed.');}
});
