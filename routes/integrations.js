...
router.get('/', authRequired, async (req, res) => {
  ...
  catch (error) { console.error('[Integrations] list:', error); res.status(500).json({ ok:false, error:'Unable to load integrations' }); }
});
router.post('/', authRequired, adminRequired, express.json(), async (req, res) => {
  ...
  } catch (error) { console.error('[Integrations] create:', error); res.status(500).json({ok:false,error:'Unable to create integration'}); }
});
router.patch('/:id', authRequired, adminRequired, express.json(), async (req,res) => {
  ...
  } catch (error) { console.error('[Integrations] update:', error); res.status(500).json({ok:false,error:'Unable to update integration'}); }
});
router.delete('/:id', authRequired, adminRequired, async (req,res) => {
  ...
  catch (error) { console.error('[Integrations] delete:', error); res.status(500).json({ok:false,error:'Unable to delete integration'}); }
});
